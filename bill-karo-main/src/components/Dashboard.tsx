...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  useEffect(() => {
    async function loadRealTimeDashboard() {
      ...
      try {
        ...
        if (userErr || !user) {
          // No active Supabase session, fallback to local state
        ...
        // Fetch client details, invoices, and expense transactions in parallel
        ...
        if (clientsRes.error || invoicesRes.error) {
          // If query fails, fall back to local store
          setSupabaseMode(false);
          setLoading(false);
          return;
        }
        ...
        // Compute metrics from Supabase
        // A. Total pending amount (from unpaid / partially paid invoices or client ledger)
        const pendingAmount = dbInvoices
          .filter((inv: any) => inv.status !== 'Paid')
          .reduce((sum: number, inv: any) => {
            const grandTotal = Number(inv.grand_total || 0);
            // Calculate sum of payments from payments jsonb column safely
            let paidSum = 0;
        ...
        // B. This Month Income (sum of all expenses/transactions of type 'income' or payments made this month)
        ...
        // F. Recent Invoices with mapped client details
        ...
        console.warn('Silent fallback triggered. Connecting to local server store instead.', err);
        setSupabaseMode(false);
      } finally {
        setLoading(false);
      }
  ...
  // Fallback Calculations using Zustand Store if mode is local
  ...
  // Recent 5 Invoices from Zustand Store
  ...
  // Decide current values to render
  ...
  const todayDeadlines = supabaseMode ? sbStats.todayDeadlines : []; // deadlines only populated on custom supabase fields
  ...
  /**
   * Status Pillar Color Configuration
   * Traffic Light System: Green=Paid, Yellow=Partial, Red=Unpaid/Overdue
   */
  ...
  return (
    <div className="space-y-6">
      {/* 1. Header Banner & Sync state indicator */}
      ...
      {/* 2. CORE STATS PANEL (Row of 4 Key Cards) */}
      ...
      {/* 5. RECENT INVOICES WITH STATUS COLORS */}
